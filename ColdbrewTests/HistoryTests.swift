...
class HistoryTests: XCTestCase { // swiftlint:disable:this type_body_length
  ...
  func testAddingItemThatIsSupersededByExisting() throws {
    ...
    firstItem.application = "Coldbrew.app"
    ...
    secondItem.application = "Coldbrew.app"
  ...
  func testAddingItemFromColdbrew() {
    let firstContents = [
      HistoryItemContent(
        type: NSPasteboard.PasteboardType.string.rawValue,
        value: "one".data(using: .utf8)
    ...
    let secondContents = [
      ...
      HistoryItemContent(
        type: NSPasteboard.PasteboardType.fromColdbrew.rawValue,
        value: "".data(using: .utf8)
      )
    ]
    let second = HistoryItem()
    Storage.shared.context.insert(second)
    second.application = "Coldbrew.app"
