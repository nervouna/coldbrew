...
class ClipboardTests: XCTestCase {
  ...
  let customType = NSPasteboard.PasteboardType(rawValue: "org.coldbrew.ConfidentialType")
  ...
  func testCopy() {
    ...
    XCTAssertEqual(pasteboard.string(forType: .fromColdbrew), "")
    XCTAssertEqual(pasteboard.string(forType: .source), "com.foo.bar")
  }

  @MainActor
  func testCopyString() {
    clipboard.copyInColdbrew("foo")
    XCTAssertEqual(pasteboard.string(forType: .string), "foo")
    XCTAssertEqual(pasteboard.string(forType: .source), NSPasteboard.PasteboardType.fromColdbrew.rawValue)
  }
