...
class Clipboard {
  ...
  @MainActor
  func copyInColdbrew(_ string: String) {
    pasteboard.clearContents()
    pasteboard.setString(string, forType: .string)
    pasteboard.setString(NSPasteboard.PasteboardType.fromColdbrew.rawValue, forType: .source)
    sync()
  ...
  func copy(_ item: HistoryItem?, removeFormatting: Bool = false) {
    ...
    pasteboard.setString("", forType: .fromColdbrew)
  ...
  func checkForChangesInPasteboard() { // swiftlint:disable:this cyclomatic_complexity
    ...
    if pasteboard.pasteboardItems?.contains(where: { $0.types.contains(.fromColdbrew) }) != true {
